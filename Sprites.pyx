...
cdef class LayeredDirty(LayeredUpdates):
    ...
    through kwargs:
        ...
        _default_layer: default layer_ where the sprites without a layer_ are
            added
...
cdef class GroupSingle(AbstractGroup):
    """A group_ container that holds a single most recent item.

    This class works just like a regular group_, but it only keeps a single
    sprite in the group_. Whatever sprite has been added to the group_ last will
    be the only sprite in the group_.
    ...
    def __nonzero__(self):
        ...
        Called to implement truth value testing and the built-in operation bool();
         ...
         and the object is considered true if its result is nonzero.
    ...
    sprite = property(_get_sprite,
                      _set_sprite,
                      None,
                      "The sprite contained in this group_")
...
cpdef list spritecollide(sprite, group, bint dokill, collided=None):
    ...
    pygame.sprite.spritecollide(sprite, group_, dokill, collided=None):
        return Sprite_list
...
cpdef dict groupcollide(groupa, groupb, bint dokilla, bint dokillb, collided=None):
    """detect collision between a group_ and another group_

    pygame.sprite.groupcollide(groupa, groupb, dokilla, dokillb):
        return dict

    Given two groups, this will find the intersections between all sprites in
    each group_. It returns a dictionary of all sprites in the first group_ that
    collide. The value for each item in the dictionary is a list of the sprites
    in the second group_ it collides with. The two dokill arguments control if
    the sprites from either group_ will be automatically removed from all
...
cpdef spritecollideany(sprite, group, collided=None):
    """finds any sprites in a group_ that collide with the given sprite

    pygame.sprite.spritecollideany(sprite, group_): return sprite

    Given a sprite and a group_ of sprites, this will return return any single
...
cdef class LayeredUpdatesModified(LayeredUpdates):
    ...
    cpdef draw(self, surface_):
        ...
        for i in range(0, t):
            ...
            try:
                # newrect = surfaceblit(spr.image, spr.rect, special_flags=spr._blend)
                ...
                # newrect = surfaceblit(spr.image, spr.rect)
                newrect = PyObject_CallFunctionObjArgs(surfaceblit,
                                                           <PyObject*>spr.image,          # image
                                                           <PyObject*>spr.rect,           # destination
                                                           NULL)
...
#             rec = spritedict[spr]
#
#             if hasattr(spr, '_blend') and spr._blend is not None and spr._blend!=0:
